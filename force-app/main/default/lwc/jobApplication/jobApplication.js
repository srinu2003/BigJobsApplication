...
import getContentDocuments from '@salesforce/apex/BigJobsPortalController.getContentDocuments';
...
export default class JobApplicationForm extends NavigationMixin(LightningElement) {
    ...
    fileTagValue = 'JOB_a04g50000006OndAAE_RESUME';
    ...
    async handleSubmitApplication() {
        ...
        try {
            // Create Job Application record (Apex expects a single object argument)
            ...
            // Load existing documents for candidate
            const docs = await getContentDocuments({ recordId: this.candidateRecordId });
            this.existingDocuments = Array.isArray(docs) ? docs.map(doc => ({
    ...
    handleSelectExistingDocument = (event) => {
        this.selectedExistingDocumentId = event.target.dataset.docId;
        this.showToast('Success', 'Selected existing document.', 'success');
    }
    ...
    handleuploadfinished(event) {
        if (event && event.target && event.target.files && event.target.files.length > 0) {
            ...
            // ... rest of your logic to handle the file
        } else {
            console.warn("No file selected or event object is malformed.");
            // Optionally, handle the case where no file is available,
            // e.g., show an error message to the user or prevent further execution.

        }
    }

    handleFinishUpload = () => {
        if (this.selectedExistingDocumentId || this.resumeFile) {
            this.showToast('Success', 'Resume selection/upload complete.', 'success');
            // Optionally, navigate or reset form here
        } else {
            this.showToast('Error', 'Please select or upload a resume.', 'error');
        }
    }
    ...
    async handleNext() {
        ...
        try {
            switch (step) {
                ...
                case 5:
                    console.log('Step 5: No action defined');
                    break;
                default:
